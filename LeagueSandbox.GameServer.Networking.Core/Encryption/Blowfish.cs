...
namespace LeagueSandbox.GameServer.Networking.Core.Encryption
{
    public class Blowfish
    {
        ...
        private void SetupKey(byte[] cipherKey)
        {
            ...
            var blockData = new BlockData(0, 0);
            for (var i = 0; i < 18; i += 2)
            {
                blockData = encipher(blockData);
                bf_P[i] = blockData.xl_par;
                bf_P[i + 1] = blockData.xr_par;
            }

            for (var i = 0; i < 256; i += 2)
            {
                blockData = encipher(blockData);
                bf_s0[i] = blockData.xl_par;
                bf_s0[i + 1] = blockData.xr_par;
            }
            for (var i = 0; i < 256; i += 2)
            {
                blockData = encipher(blockData);
                bf_s1[i] = blockData.xl_par;
                bf_s1[i + 1] = blockData.xr_par;
            }
            for (var i = 0; i < 256; i += 2)
            {
                blockData = encipher(blockData);
                bf_s2[i] = blockData.xl_par;
                bf_s2[i + 1] = blockData.xr_par;
            }
            for (var i = 0; i < 256; i += 2)
            {
                blockData = encipher(blockData);
                bf_s3[i] = blockData.xl_par;
                bf_s3[i + 1] = blockData.xr_par;
            }
        }
        ...
        {
            var blockData = SetBlock(block);
            blockData = encipher(blockData);
            GetBlock(blockData, ref block);
        }
        ...
        private BlockData SetBlock(byte[] block)
        {
            ...
            var xl_par = BitConverter.ToUInt32(block1, 0);
            var xr_par = BitConverter.ToUInt32(block2, 0);
            return new BlockData(xl_par, xr_par);
        }
        ...
        private void GetBlock(BlockData blockData, ref byte[] block)
        {
            var block1 = BitConverter.GetBytes(blockData.xl_par);
            var block2 = BitConverter.GetBytes(blockData.xr_par);
        ...
        private BlockData encipher(BlockData blockData)
        {
            var xl_par = blockData.xl_par;
            var xr_par = blockData.xr_par;
        ...
        /// </summary>
        /// <param name="blockData"></param>
        private BlockData decipher(BlockData blockData)
        {
            var xl_par = blockData.xl_par;
            var xr_par = blockData.xr_par;
            xl_par ^= bf_P[17];
        ...
        {
            public uint xl_par { get; }
            public uint xr_par { get; }

            public BlockData(uint xlPar, uint xrPar)
            {
                xl_par = xlPar;
                xr_par = xrPar;
            }
